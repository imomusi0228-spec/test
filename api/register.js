...
// Vercel Serverless Function: GET /api/register?name=名前&comment=コメント&vrcid=ID
module.exports = async (req, res) => {
  ...
  const { name, comment = '', vrcid = '' } = req.query;
  ...
  try {
    ...
    if (targetGuest) {
      ...
      // 新規顧客 (基本は /api/join で登録済みのはずだが、フォールバック)
      const notes = comment ? [{
        id: 'note-' + Date.now(),
        date: today,
    ...
    // 2. リアルタイムイベントログに書き込み (ダッシュボード用)
    const { error: eventError } = await supabase
      .from('realtime_events')
      .insert({
        type: 'guestbook-register', // Webダッシュボードがこのtypeで検知してカルテを開くため、互換性を維持
        player_name: name,
        comment: comment,
    ...
    // 3. 最新の名簿一覧データを取得して、UdonSharp形式で返却
